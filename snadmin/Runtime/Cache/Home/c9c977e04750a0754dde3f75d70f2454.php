...
$(document).ready(function() {
	$(".delbut").click(function() {
		if (confirm("是否删除该广告?")) {
			$.post("/admin.php/Admin/Adv/delAdv/aId/"+ this.id, function(data) {
				if (data > 0) {
					alert("删除成功！");
...
<body>
	<div class="place">
		<span>位置：</span>
		<ul class="placeul">
			<li><a href="javascript:void(0);">抽奖管理</a></li>
			<li><a href="javascript:void(0);">抽奖设置列表</a></li>
		</ul>
	</div>
	...
	<table  class="tablelist" style="margin-top:3px;">
		...
		<td><?php echo ($news["aid"]); ?></td>
			<td><?php echo ($news["aname"]); ?></td>
			<td>
				<?php if($news["atype"] == 1): ?>余额奖励<?php endif; ?>
				<?php if($news["atype"] == 2): ?>排单卷奖励<?php endif; ?>
			</td>
			<td><?php echo ($news["num"]); ?></td>
			<td><?php echo ($news["addtime"]); ?></td>
			<td>
				<a href="/admin.php/Home/Award/addAward/aId/<?php echo ($news["aid"]); ?>">编辑</a>  
