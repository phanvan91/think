...
function ppdd_add($p_id,$g_id){
	 ...
	 $get_user=$g_user1['user'];
	 $supplier=$p_user1['user'];
	 $supplier_phone=M('user')->where("UE_account='{$supplier}'")->getField('UE_phone');
	 $get_user_phone=M('user')->where("UE_account='{$get_user}'")->getField('UE_phone');
	 $content="您好，{$supplier}！您提供的帮助{$ppddxx['jb']}已经匹配！";
	 $content1="您好，{$get_user}！您接受的帮助{$ppddxx['jb']}已经匹配！";
	 $url='http://utf8.sms.webchinese.cn/?Uid=qq112255&Key=4d3d66c37f377d703dd9&smsMob='.$supplier_phone.'&smsText='.$content;

	 $returnval=Getmobile1($url);
				
	 $url1='http://utf8.sms.webchinese.cn/?Uid=qq112255&Key=4d3d66c37f377d703dd9&smsMob='.$get_user_phone.'&smsText='.$content1;
	 $returnval1=Getmobile1($url1);
