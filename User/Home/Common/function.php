...
function user_jj_lx($var)
{
    ...
    if($tgbzinfo['dakuantype'] == 3)
    {
        $lixi_total = ($proall['jb'] * $jjdjdays * 0.015);
    }
    else if($tgbzinfo['dakuantype'] == 2)
    {
       ...
       // $diff2 = diffBetweenTwoDays($NowTime2, $nowtime3);//计算配对时间到提现的天数      配对-----------提现时间
        ...
        $lixi_total = ($proall['jb'] * $diff1 * (intval(C("lixi1")) / 100)) + ($proall['jb'] * 1* (intval(C("lixi2")) / 100));
    }
    else
    {
        $NowTime = date('Y-m-d', strtotime($proall['date']));//格式化加入时间  确认打开  获取提供帮助者排单时间
       ...
       // $diff2 = diffBetweenTwoDays($NowTime2, $nowtime3);//计算配对时间到提现的天数      配对-----------提现时间
        ...
        $lixi_total = ($proall['jb'] * $diff1 * (intval(C("lixi1")) / 100)) + ($proall['jb'] * 1* (intval(C("lixi1")) / 100));
